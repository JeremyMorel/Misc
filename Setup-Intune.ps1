...
#>
    ...
    try
    {
        ...
        $EnterpriseDomain = ($domains | Where-Object { $_.isInitial -eq $true } | Select-Object id).id
...
{
    Write-Host "MDM policies for MacOS will not be imported" -ForegroundColor Red
    Write-Host
}
####################################################
